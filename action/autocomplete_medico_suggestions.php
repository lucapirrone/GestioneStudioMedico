...
/*
	...
	*/
    ...
    if(checkValidate($array_info, $conn)) { 
		...
		if ($stmt_utente = $conn->prepare("SELECT  ID, NOME, COGNOME, COD_FISCALE, INDIRIZZO, CAP, CITTA FROM MEDICI where KCO = ? and CONCAT(COD_FISCALE, ' ', COGNOME, ' ', NOME) LIKE ?")){
			...
			if($stmt_utente->execute()){ // esegue la query appena creata.
				...
				$stmt_utente->bind_result($id, $nome, $cognome, $codicefiscale, $indirizzo, $cap, $citta);
				...
				while($stmt_utente->fetch()) {
					...
					if($nome!=null && $cognome!=null) $label = $nome." ".$cognome;
